...
FText SSMLicenseDialog::GetLicenseTypeText() const
{
	...
	{
		return LOCTEXT("MarketplaceLicense", "Marketplace (Logic Driver Pro Indie)");
	}
...
"Marketplace code plugins are sold on a per-user basis, as stipulated by the Epic Games content license agreement.\
\nEvery individual who uses the plugin is required to hold a valid license.\
\n\nThis automated license check is to assist studios which might not be aware of their licensing status.\
\nIf you have any questions or wish to discuss licensing solutions for your studio, please contact Recursoft.\
\n\nLogic Driver Pro is currently available on the Marketplace so small indies can easily access the software.\
\nPlease check the latest information on availability as offerings may change.");
	}
	else
	{
...
\n- An enterprise or custom license with Recursoft.\
\n- An Epic Games per-user Marketplace license with GitHub access intended for small indies.\
